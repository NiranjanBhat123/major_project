...
class SubServiceAdmin(admin.ModelAdmin):
    list_display = ('name', 'image_preview', 'main_service', 'providers_count', 'avg_price')
    list_filter = ('main_service', 'created_at')
    search_fields = ('name', 'main_service__name')
    readonly_fields = ('id', 'created_at', 'updated_at', 'providers_count', 'avg_price', 'image_preview')
    ordering = ('main_service', 'name')
    
    fieldsets = (
        (_('Basic Information'), {
            'fields': ('id', 'name', 'main_service', 'image', 'image_preview')
        }),
        (_('Statistics'), {
